...
class ExogenousVariableEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        """
        x: [B, T, V] → for example, [64, 50, 11]
        returns: [B, V, hidden_dim] – token per variable
        """
        B, T, V = x.size()
        x = x.transpose(1, 2)  # → [B, V, T]
        var_tokens = []
        for i, encoder in enumerate(self.var_encoders):
            var_i = x[:, i:i+1, :].transpose(1, 2)  # [B, T, 1]
            var_token = encoder(var_i.mean(dim=1, keepdim=True))  # [B, 1, D]
            var_tokens.append(var_token)
        return torch.cat(var_tokens, dim=1)  # [B, V, D]
...
class CrossAttentionModule(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key_value: torch.Tensor):
        """
        query: [B, T, D]  (decoder output)
        key_value: [B, V, D] (variable tokens)
        return: [B, T, D]
        """
        attn_output, attn_weights = self.cross_attn(query=query, key=key_value, value=key_value)
    ...
    **kwargs: additional arguments to :py:class:`~BaseModel`.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.exog_encoder = ExogenousVariableEncoder(input_dim=1, hidden_dim=hidden_size, num_vars=11)
        self.cross_attn_module = CrossAttentionModule(d_model=hidden_size, n_heads=attention_head_size)
    ...
    def forward(self, x: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        # print(f"[Debug] encoder_lengths shape: {encoder_lengths.shape}, max: {encoder_lengths.max()}")
        # print(f"[Debug] decoder_lengths shape: {decoder_lengths.shape}, max: {decoder_lengths.max()}")
        ...
        # print(f"[Debug] x_cat shape: {x_cat.shape}")
        # print(f"[Debug] x_cont shape: {x_cont.shape}")
        ...
        # print("[Debug] encoder input variable names:", list(embeddings_varying_encoder.keys()))
        embeddings_varying_encoder, encoder_sparse_weights = (
            self.encoder_variable_selection(
                embeddings_varying_encoder, context=None
            )
        )
        # print(f"[Debug] encoder_sparse_weights shape: {encoder_sparse_weights.shape}")
        embeddings_varying_decoder = {
            name: input_vectors[name][:, max_encoder_length:]
            for name in self.decoder_variables  # select decoder
        }
        
        # print("[Debug] decoder input variable names:", list(embeddings_varying_decoder.keys()))
        embeddings_varying_decoder, decoder_sparse_weights = (
            self.decoder_variable_selection(
               embeddings_varying_decoder, context=None
            )
        )
        # print(f"[Debug] decoder_sparse_weights shape: {decoder_sparse_weights.shape}")
        # LSTM
        ...
        # print(f"[Debug] input_hidden shape: {input_hidden.shape}")
        # print(f"[Debug] input_cell shape: {input_cell.shape}")
        ...
        # print(f"[Debug] decoder_output shape before cross-attn: {decoder_output.shape}")
        num_exog_vars = 11  # or dynamically infer based on your setup
        exog_input = x["decoder_cont"][..., :num_exog_vars]  # shape: [B, T, V]
        exog_tokens = self.exog_encoder(exog_input)          # shape: [B, V, D]
        decoder_output, cross_attn_weights = self.cross_attn_module(decoder_output, exog_tokens)
        # print(f"[Debug] exog_input shape: {exog_input.shape}")
        # print(f"[Debug] exog_tokens shape: {exog_tokens.shape}")
        # print(f"[Debug] decoder_output shape after cross-attn: {decoder_output.shape}")
        ...
        # print(f"[Debug] concatenated lstm_output shape: {lstm_output.shape}")
        # static enrichment
        ...
        # print(f"[Debug] attn_output shape: {attn_output.shape}")
        # print(f"[Debug] attn_output_weights shape: {attn_output_weights.shape}")
        ...
        # print(f"[Debug] output shape after fusion decoder: {output.shape}")
        if self.n_targets > 1:  # if to use multi-target architecture
            output = [output_layer(output) for output_layer in self.output_layer]
            # print(f"[Debug] multi-target output shapes: {[o.shape for o in output]}")
        else:
            output = self.output_layer(output)
            # print(f"[Debug] single-target output shape: {output.shape}")
