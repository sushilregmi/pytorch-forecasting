...
class CrossVariableAttention(nn.Module):
    """
    Cross-attends across the variable dimension of a [B, T, V, D] tensor
    and returns a fused [B, T, V, D] tensor.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: [B, T, V, D]
        B, T, V, D = x.shape
        # merge batch/time: treat each (b,t) as its own sequence of V tokens
        x_flat = x.view(B * T, V, D)                                   # [B·T, V, D]
        qkv = self.qkv(x_flat)                                        # [B·T, V, 3D]
        q, k, v = qkv.chunk(3, dim=-1)
        # reshape to heads
        def split_heads(z):
            return z.view(B * T, V, self.n_heads, self.d_head).transpose(1,2)  # [B·T, H, V, d_h]
        q, k, v = map(split_heads, (q,k,v))
        # scaled dot-product
        scores = (q @ k.transpose(-2,-1)) * self.scale               # [B·T, H, V, V]
        weights = torch.softmax(scores, dim=-1)
        attn = (weights @ v)                                          # [B·T, H, V, d_h]
        attn = attn.transpose(1,2).contiguous().view(B * T, V, D)     # [B·T, V, D]
        out = self.out(self.dropout(attn))                            # [B·T, V, D]
        return out.view(B, T, V, D)       
    ...
    **kwargs: additional arguments to :py:class:`~BaseModel`.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cross_variable_attn_encoder = CrossVariableAttention(
    d_model=self.hparams.hidden_size,
    n_heads=self.hparams.attention_head_size,
    dropout=self.hparams.dropout,
)
        
        self.cross_variable_attn_decoder = CrossVariableAttention(
    d_model=self.hparams.hidden_size,
    n_heads=self.hparams.attention_head_size,
    dropout=self.hparams.dropout,
)
...
# 2) call VSN once, just to get sparse weights
        _, encoder_sparse_weights = self.encoder_variable_selection(
    raw_enc,
    static_context_variable_selection[:, :max_encoder_length]
)

# 3) rebuild each var’s processed embedding
        var_enc_outputs = []
        for name in self.encoder_variables:
            x_var = raw_enc[name]
            if name in self.encoder_variable_selection.prescalers:
                x_var = self.encoder_variable_selection.prescalers[name](x_var)
            grn = self.encoder_variable_selection.single_variable_grns[name]
            x_var = grn(x_var)
            var_enc_outputs.append(x_var)              # now x_var is [B, T_enc, D]

# 4) cross-variable attention + fuse
        enc_stack = torch.stack(var_enc_outputs, dim=2)  # [B, T_enc, V, D]
        enc_stack = self.cross_variable_attn_encoder(enc_stack)
        encoder_input = enc_stack.mean(dim=2)            # [B, T_enc, D]

# ——— do the exact same on the decoder ———

        raw_dec = {
    name: input_vectors[name][:, max_encoder_length:]
    for name in self.decoder_variables
}
        _, decoder_sparse_weights = self.decoder_variable_selection(
    raw_dec,
    static_context_variable_selection[:, max_encoder_length:]
)
        ...
        for name in self.decoder_variables:
            x_var = raw_dec[name]
            if name in self.decoder_variable_selection.prescalers:
                x_var = self.decoder_variable_selection.prescalers[name](x_var)

            grn = self.decoder_variable_selection.single_variable_grns[name]
            x_var = grn(x_var)
            var_dec_outputs.append(x_var)

        dec_stack = torch.stack(var_dec_outputs, dim=2)
        dec_stack = self.cross_variable_attn_decoder(dec_stack)
        decoder_input = dec_stack.mean(dim=2) 
    ...
    encoder_input, (input_hidden, input_cell),
    lengths=encoder_lengths, enforce_sorted=False
)
        decoder_output, _ = self.lstm_decoder(
    decoder_input, (hidden, cell),
    lengths=decoder_lengths, enforce_sorted=False
)

        lstm_skip_enc = encoder_input   # no dict here!
        lstm_skip_dec = decoder_input
        # skip connection over lstm
        lstm_output_encoder = self.post_lstm_gate_encoder(encoder_output)
        lstm_output_encoder = self.post_lstm_add_norm_encoder(
             lstm_output_encoder, lstm_skip_enc
        )

        lstm_output_decoder = self.post_lstm_gate_decoder(decoder_output)
        lstm_output_decoder = self.post_lstm_add_norm_decoder(
            lstm_output_decoder, lstm_skip_dec
        )
