...
class MoEOutputLayer(nn.Module):
    def __init__(self, input_dim, output_dim, num_experts=2, hidden_dim=128):
        super().__init__()
    ...
    def forward(self, x):
        ...
        expert_outputs = torch.stack([expert(x) for expert in self.experts], dim=-1)  # [B, T, output_dim, num_experts]

        # Weighted sum: sum over experts
        output = torch.sum(expert_outputs * gate_weights.unsqueeze(-2), dim=-1)  # [B, T, output_dim]
        return output
...
class TemporalFusionTransformer(BaseModelWithCovariates):
    ...
    **kwargs: additional arguments to :py:class:`~BaseModel`.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        MoEOutputLayer(
            ...
            num_experts=2,  # or whatever you choose
            hidden_dim=128
        )
        for output_size in self.hparams.output_size
    ])
        else:
            self.output_layer = MoEOutputLayer(
        input_dim=self.hparams.hidden_size,
        output_dim=self.hparams.output_size,
        num_experts=2,
        hidden_dim=128
    )
    @classmethod
    ...
    def forward(self, x: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        if self.n_targets > 1:  # Multi-target case
            output = [output_layer(output) for output_layer in self.output_layer]  # List of MoE outputs
            output = torch.cat(output, dim=-1)  # Concatenate along feature dim (assumes same shape per target)
        else:
            output = self.output_layer(output)  # MoEOutputLayer returns [B, T, output_dim]
