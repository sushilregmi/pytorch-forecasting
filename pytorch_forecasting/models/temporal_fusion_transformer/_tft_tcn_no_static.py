...
class Chomp1d(nn.Module):
    """
    A module that removes the last `chomp_size` elements from the time dimension of a tensor.
    Used to implement causal convolutions.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x (torch.Tensor): Tensor of shape (batch_size, channels, sequence_length).
        Returns:
            torch.Tensor: Tensor of shape (batch_size, channels, sequence_length - chomp_size).
        """
        return x[:, :, : -self.chomp_size].contiguous()

class TemporalBlock(nn.Module):
    """
    A residual block for the Temporal Convolutional Network.
    It consists of two dilated causal convolutional layers with ReLU activations,
    weight normalization, and dropout. A residual connection is added to the output.
    """
    def __init__(
        self,
        n_inputs: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x (torch.Tensor): Input tensor of shape (batch_size, n_inputs, sequence_length).
        Returns:
...
class TemporalConvNet(nn.Module):
    """
    A Temporal Convolutional Network (TCN) composed of a stack of TemporalBlocks.
    The dilation factor increases exponentially with each layer.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x (torch.Tensor): Input tensor of shape (batch_size, num_inputs, sequence_length).
        Returns:
            torch.Tensor: Output tensor of shape (batch_size, num_channels[-1], sequence_length).
        """
...
class TemporalFusionTransformerTCNNoStatic(BaseModelWithCovariates):
    """Temporal Fusion Transformer variant with TemporalConvNet + no static covariates.
    ...
    **kwargs: additional arguments to :py:class:`~BaseModel`.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #TCN encoder and decoder implementation

        self.tcn_channels = [self.hparams.hidden_size] * self.hparams.tcn_layers
        self.tcn_encoder = TemporalConvNet(
            num_inputs=self.hparams.hidden_size,
            num_channels=self.tcn_channels,
            kernel_size=self.hparams.tcn_kernel_size,
            dropout=self.hparams.tcn_dropout,
        )

        self.tcn_decoder = TemporalConvNet(
            num_inputs=self.hparams.hidden_size,
            num_channels=self.tcn_channels,
            kernel_size=self.hparams.tcn_kernel_size,
            dropout=self.hparams.tcn_dropout,
        ...
        self.post_tcn_gate_encoder = GatedLinearUnit(
            self.hparams.hidden_size, dropout=self.hparams.dropout
        )
        self.post_tcn_gate_decoder = self.post_tcn_gate_encoder
        self.post_tcn_add_norm_encoder = AddNorm(
            self.hparams.hidden_size, trainable_add=False
        )
        self.post_tcn_add_norm_decoder = self.post_tcn_add_norm_encoder
    ...
    def forward(self, x: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        embeddings_varying_encoder, encoder_sparse_weights = (
            self.encoder_variable_selection(
                embeddings_varying_encoder, context=None
            )
        )
        ...
        embeddings_varying_decoder, decoder_sparse_weights = (
            self.decoder_variable_selection(
               embeddings_varying_decoder, context=None
            )
        )
       ...
       # tcn encoder and decoder
        
        encoder_output = self.tcn_encoder(embeddings_varying_encoder.permute(0, 2, 1))
        encoder_output = encoder_output.permute(0, 2, 1) # permute back to (batch, time, channels)

        # run local decoder
        decoder_output = self.tcn_decoder(embeddings_varying_decoder.permute(0, 2, 1))
        decoder_output = decoder_output.permute(0, 2, 1) # permute back to (batch, time, channels)

        # skip connection over tcn
        tcn_output_encoder = self.post_tcn_gate_encoder(encoder_output)
        tcn_output_encoder = self.post_tcn_add_norm_encoder(
            tcn_output_encoder, embeddings_varying_encoder
        )

        tcn_output_decoder = self.post_tcn_gate_decoder(decoder_output)
        tcn_output_decoder = self.post_tcn_add_norm_decoder(
            tcn_output_decoder, embeddings_varying_decoder
        )

        tcn_output = torch.cat([tcn_output_encoder, tcn_output_decoder], dim=1)

        temporal_context = self.temporal_context_embedding.expand(batch_size, timesteps, -1)
        attn_input = tcn_output + temporal_context
        ...
        output = self.pre_output_gate_norm(output, tcn_output[:, max_encoder_length:])
