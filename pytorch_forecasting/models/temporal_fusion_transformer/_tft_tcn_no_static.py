...
class TemporalFusionTransformer(BaseModelWithCovariates):
    ...
    **kwargs: additional arguments to :py:class:`~BaseModel`.
    ...
    def forward(self, x: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        x_enc = embeddings_varying_encoder.transpose(1, 2).contiguous()  # (B, C, T)
        encoder_output = self.tcn_encoder(x_enc)
        encoder_output = encoder_output.transpose(1, 2)                  # (B, T, C)

# Decoder
        x_dec = embeddings_varying_decoder.transpose(1, 2).contiguous()  # (B, C, T)
        decoder_output = self.tcn_decoder(x_dec)
        decoder_output = decoder_output.transpose(1, 2)                  # (B, T, C)
