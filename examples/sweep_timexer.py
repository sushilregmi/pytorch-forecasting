...
LOG_FILE = "timexer_multirun_lon.log"             # append-only
CKPT_BASE_DIR = "checkpoints_timexer_norm_multirun_lon"
...
def parse_epoch_from_ckpt_name(ckpt_path: str) -> int:
    ...
    m = re.search(r"best-([0-9]+)-", base)  # best-07-0.1234.ckpt
    if m:
        return int(m.group(1))
...
def log_header(f, run_id: int, horizon: int):
    f.write("\n" + "=" * 120 + "\n")
    f.write(f"MODEL=TimeXer | RUN={run_id} | HORIZON={horizon} | ts={time.strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("=" * 120 + "\n")
    f.flush()
...
class EpochTimeCollector(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        print(f"[TimeXer run={self.run_id} H={self.horizon}] Epoch={trainer.current_epoch:02d} time(train+val)={dt:.3f}s")
        self._t0 = None
...
for run_id in range(1, RUNS + 1):
    for horizon in HORIZONS:
        ...
        with open(LOG_FILE, "a", buffering=1) as log_f, redirect_stdout(log_f), redirect_stderr(log_f):
            ...
            print(f"\nEND | TimeXer run={run_id} horizon={horizon}")
...
summary_df.to_csv("timexer_multirun_per_run.csv", index=False)
agg_df.to_csv("timexer_multirun_by_horizon.csv", index=False)

with open(LOG_FILE, "a", buffering=1) as f:
    f.write("\n" + "#" * 120 + "\n")
    f.write("FINAL SUMMARY (TimeXer) - per run\n")
    f.write("#" * 120 + "\n")
    f.write(summary_df.to_string(index=False))
    f.write("\n\n" + "#" * 120 + "\n")
    f.write("FINAL SUMMARY (TimeXer) - mean/std by horizon\n")
    f.write("#" * 120 + "\n")
